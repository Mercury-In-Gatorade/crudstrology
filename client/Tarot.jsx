  ...
  return (
    <div>
      ...
      <div style={{ fontSize: '20px' }}>
        <b>
          <p>Your fate will be told...</p>
        </b>
      </div>
      <p></p>
      <Button variant='secondary' onClick={drawCards}>
      ...
      <Reading>
        {' '}
        <b>
          The interpretation of your reading... Past:{' '}
          {tarot.length > 0 && <u>{tarot[0].name}</u>}, Present:{' '}
          {tarot.length > 0 && <u>{tarot[1].name}</u>}, Future:{' '}
          {tarot.length > 0 && <u>{tarot[2].name}</u>}
        </b>
        <div>
          {fortune.map((el, i) => (
            <span key={i}>
            ...
            <TarotCard key={i}>
              ...
              <div key={i + 1}>
                {Object.entries(card).map((el, i) => {
                  return (
                    <div style={{ fontSize: '18px' }} key={i}>
                      <b>{el[0] === 'desc' ? <u>{el[0]}</u> : el[0]}: </b>
                      {el[0] === 'desc' ? <b>{el[1]}</b> : <em>{el[1]}</em>}
                    </div>
                  );
                })}
              </div>
